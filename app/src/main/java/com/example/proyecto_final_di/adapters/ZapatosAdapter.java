...
public class ZapatosAdapter extends RecyclerView.Adapter<ZapatosAdapter.ViewHolder> {

    private List<ItemGeneral> datosZapatos;
    ...
    public ZapatosAdapter(List<ItemGeneral> datosZapatos) {
        this.datosZapatos = datosZapatos;
    }


    @NonNull
    @Override
    public ZapatosAdapter.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {

        View view = LayoutInflater.from(viewGroup.getContext())
                .inflate(R.layout.item_general, viewGroup, false);

        return new ZapatosAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(ZapatosAdapter.ViewHolder viewHolder, final  int position) {

        ItemGeneral item = datosZapatos.get(position);
        ...
        viewHolder.getImg().setImageResource(datosZapatos.get(position).getImg());
        viewHolder.getNombreTextView().setText(datosZapatos.get(position).getNombre());
        viewHolder.getPrecioTextView().setText(String.valueOf(datosZapatos.get(position).getPrecio()));


        ItemGeneral item2 = datosZapatos.get(position);
