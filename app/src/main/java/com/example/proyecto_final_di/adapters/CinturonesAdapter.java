...
public class CinturonesAdapter extends RecyclerView.Adapter<CinturonesAdapter.ViewHolder> {

    private List<ItemGeneral> datosCinturones;
    ...
    public CinturonesAdapter(List<ItemGeneral> datosCinturones) {
        this.datosCinturones = datosCinturones;
    }


    @NonNull
    @Override
    public CinturonesAdapter.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {

        View view = LayoutInflater.from(viewGroup.getContext())
                .inflate(R.layout.item_general, viewGroup, false);

        return new CinturonesAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(CinturonesAdapter.ViewHolder viewHolder, final  int position) {

        ItemGeneral item = datosCinturones.get(position);
        ...
        viewHolder.getImg().setImageResource(datosCinturones.get(position).getImg());
        viewHolder.getNombreTextView().setText(datosCinturones.get(position).getNombre());
        viewHolder.getPrecioTextView().setText(String.valueOf(datosCinturones.get(position).getPrecio()));


        ItemGeneral item2 = datosCinturones.get(position);
